...
Infrastructure Overview: Instance counts, health status
Performance Metrics: CPU, Memory, Disk time-series
Healing Activity: Lambda invocation count, execution time
Regional Analysis: Instance distribution across regions
Best Practices & Security
...
Verify SSM Agent on Target Instance:

sudo systemctl status amazon-ssm-agent
Check SSM Document:

aws ssm describe-document --name "AWS-RunShellScript"
Check IAM Role on Target Instance:
...
Check CloudWatch Agent on Target Instances:
